...
const Word = React.memo(function Word({ index, word, isActive, isCut, inSel, onWordClick, onEdit }) {
    const displayText = word.emoji ? `${word.emoji} ${word.text}` : word.text;
...
export default function TranscriptPanel({ captions, framing, playerRef, onEditWord, dispatch }) {
    ...
    const rows = useMemo(() => {
        const out = [];
        let nextSeg = 0;
        const pauseByWord = new Map(visibleTranscriptPauses(captions).map((pause) => [pause.index, pause]));
    ...
    const cutPauseKeys = useMemo(() => {
        const keys = new Set();
        visibleTranscriptPauses(captions).forEach((pause) => {
        ...
        return keys;
    }, [captions, framing, pauseToSource]);
    ...
    const filteredEmojis = useMemo(() => {
        ...
        if (!q) return EMOJI_CHOICES;
        return EMOJI_CHOICES.filter((emoji) => emoji.includes(q));
    }, [emojiQuery]);

    const insertEmoji = useCallback((emoji) => {
        const next = draft ? `${emoji} ${draft}` : emoji;
        setDraft(next);
        if (editingIndex !== null) onEditWord(editingIndex, next.trim());
        setEditingIndex(null);
        setSel(null);
        setEmojiOpen(false);
        setEmojiQuery('');
            ...
            {emojiOpen && (
                ...
                >
                    ...
                    >
                        ...
                        <div className="grid grid-cols-9 gap-1.5 overflow-y-auto custom-scrollbar pr-1 max-h-[46vh]">
