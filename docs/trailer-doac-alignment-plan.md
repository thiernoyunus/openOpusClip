...
### Phase 3 — Selects pass, sentence-index cuts, Imran captions (2026-09-23)

User report: trailers still cut people off mid-sentence, lack the "juicy" lines, the trailer's transcript panel sometimes shows the episode's opening minute, and captions don't match the Imran DOAC edit built in HyperFrames.

- **Why cuts ended mid-sentence.** Three stacked causes: (1) the model returned float seconds and routinely landed a few words short of the full stop; `_refine_trailer_moments` snapped to the *nearest* word, so the short end stuck, and `_verbatim_align_moments` only corrects at <0.85 text similarity (two missing words out of fifteen pass); (2) `_build_sentence_transcript` hard-split every 22 words, so long sentences became fake "sentences" ending mid-clause, and the model picked them as if complete; (3) the 0.15s tail pad clipped last syllables because ASR word end-times run early. Fix: the model now answers in sentence indices (`from_i`/`to_i`, `_resolve_moment_bounds`), long sentences split only at clause marks and carry `more`, `_complete_thought_bounds` widens every non-final moment to a real sentence end (punctuation, pause, or speaker change), and the tail pad is 0.28s. The final cliffhanger still cuts mid-sentence, at the last word of its `text`.
- **Selects pass.** Ant Smith first cuts the episode to its best soundbites, then scripts from those ("Meet The Viral Editor Behind Steven Bartlett"). `SOUNDBITE_PROMPT_TEMPLATE` does that over the full transcript; the trailer prompt then scripts from the selected sentences only (`TRAILER_SOUNDBITE_PASS=0` disables it). The trailer prompt now names the four parts (hook, lesson, rollercoaster with stacked hooks, cliffhanger), the host/guest volley, and asks for `power_words` per moment.
- **Transcript panel.** The trailer's retimed captions were only stored in `framing.subtitles`; switching captions off in the editor left `get_clip_transcript` slicing the episode transcript at 0..trailer length. The captions are now also saved as `shorts[0].trailer_captions`.
- **Captions.** The `doac` template now mirrors the Imran build: small Montserrat run-up, huge Anton power word on its own line (emotion colour for the accent word), Playfair italic tail, per-word blur-rise reveal, block blur-out, letter-only shadow, side-aligned when smart placement parks it beside a speaker. Smart placement defaults on and the trailer page defaults to Soniox (speaker labels) when a key is saved.
